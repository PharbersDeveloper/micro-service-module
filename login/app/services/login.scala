...
case class login()(implicit val rq: Request[model.RootObject], dbt: DBManagerModule, rd: RedisManagerModule)
    ...
    override def exec: Unit = {
        ...
        /** 查看用户是否有登录来源的访问权限 */
        val bind_user_product_data_lst = queryBindUserProduct(user_data.id)
        val product_lst_data = bind_user_product_data_lst.map(x => queryProduct(x.product_id))
        auth.product = Some(product_lst_data)
        if(auth.hasProduct(login_source).isEmpty) throw new Exception("user not exist")
    ...
    def queryBindUserProduct(user_id: String): List[bind_user_product] = {
        val rq = new request
        rq.res = "bind_user_product"
        rq.eqcond = Some(eq2c("user_id", user_id) :: Nil)
        queryMultipleObject[bind_user_product](rq)
    }
