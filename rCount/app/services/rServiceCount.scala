...
import com.pharbers.models.entity.bind_paper_region_goods_ym_report
...
case class rServiceCount ()(implicit val rq: Request[model.RootObject], dbt: DBManagerModule, rd: RedisManagerModule)
    ...
    override def prepare: Unit = {
        ...
        goodsSalesMap = {
            ...
            competLst.foreach(x => {
                val requestData = new request()
                requestData.eqcond = Some(List(requestObj.eq2c("goods_id", x), requestObj.eq2c("region_id", allRegion), requestObj.eq2c("ym", beforeYm)))
                ...
                result(x + "*" + beforeYm + "*" + allRegion) = queryObject[sales](requestData).getOrElse(new sales)
            })
            val requestData = new request()
            ...
            requestData.fmcond = Some(requestObj.fm2c(0, 100))
            requestData.res = "bind_course_region_goods_ym_sales"
    ...
    override def exec: Unit = {
        ...
        val goodsReportMap = {  // regionID
            ...
            paperinputLst.foreach(x => {
                ...
                report.potential_growth = targetSales.potential_growth
            ...
            result.foreach(x => {
                ...
                x._2.contri_index = x._2.sales_contri / x._2.potential_contri
            })
            result.toMap
        }
        ...
        val productReportMap = {  // goods_id
            val result = new mutable.HashMap[String, apmreport]()
            val goodsSales: Double = goodsSalesMap(goodsId + "*" + beforeYm +"*" + allRegion).sales
            var sum = 0.0
            competLst.foreach(x => {
                sum += goodsSalesMap(x + "*" + beforeYm +"*" + allRegion).sales
            })
            competLst.foreach(x => {
                val report = new apmreport()
                val beforeSales = goodsSalesMap(x + "*" + beforeYm +"*" + allRegion)
                if(x.equals(goodsId)){
        ...
        bind_paper_region_goods_ym_report_lst = {
            ...
            productReportMap.foreach(x => {
                resultLst = resultLst :+ update(x._2, allRegion, x._1)
            })
            resultLst
        }
    }
    ...
    override def goback: model.RootObject = {
        toJsonapi(bind_paper_region_goods_ym_report_lst)
//        null
    }
    ...
    private def getPaperScore(paperInput: paperinput): Double = {
        ...
        if(paperInput.predicted_target == 0) result += 0 else {
            ((answer.predicted_target / paperInput.predicted_target) - 1).abs match {
    ...
    private def countSales(growth: Double, regionId: String): Double = {
        goodsSalesMap(goodsId + "*" + beforeYm + "*" + regionId).sales * (1 + growth)
    }
    ...
    private def update(report: apmreport, regionId: String, goods:String): bind_paper_region_goods_ym_report ={
        ...
        if (bind == null) insert(report, regionId, goods) else {
            ...
            requestData.eqcond = Some(List(requestObj.eq2c("id", report.id)))
            requestData.upcond = Some(
                List(
                    requestObj.up2c("sales",report.sales),
                    requestObj.up2c("sales_growth",report.sales_growth),
                    requestObj.up2c("sales_contri",report.sales_contri),
                    requestObj.up2c("potential",report.potential),
                    requestObj.up2c("potential_growth",report.potential_growth),
                    requestObj.up2c("contri_index",report.potential_contri),
                    requestObj.up2c("contri_index",report.share),
                    requestObj.up2c("contri_index",report.share_change),
                    requestObj.up2c("contri_index",report.company_target),
                    requestObj.up2c("contri_index",report.achieve_rate),
                    requestObj.up2c("contri_index",report.contri_index)
                )
            )
