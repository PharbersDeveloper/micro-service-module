...
case class findCourseById()(implicit val rq: Request[model.RootObject], dbt: DBManagerModule, rd: RedisManagerModule)
    ...
    override val brick_name: String = "query multi course by request"

    implicit val db: DBTrait[TraitRequest] = dbt.queryDBInstance("client").get.asInstanceOf[DBTrait[TraitRequest]]

    var request_data: request = null
    var course_data: List[course] = null
    ...
    override def exec: Unit = course_data =
            queryMultipleObject[course](request_data)//.getOrElse(throw new Exception("Could not find specified course"))
