...
@Singleton
class
Controller @Inject()(implicit val cc: ControllerComponents,
    ...
    import services.findBindTeacherStudentTimePaper

    def routes(pkg: String, step: Int): Action[RootObject] = Action(circe.json[RootObject]) { implicit request =>
        Ok(PlayEntry().excution(findBindTeacherStudentTimePaper()).asJson)
    }
